...
const TransactionHistoryModal: React.FC<TransactionHistoryModalProps> = ({
    ...
    return (
        <Modal show={isOpen} onClose={onClose} size="lg" className="font-kanit">
                    ...
                    <div className="flex gap-1.5 p-1 bg-gray-100 rounded-xl">
                        {(["all", "in", "out"] as const).map((t) => (
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all duration-150 ${filterType === t
                                    ? t === "in"
                    ...
                    {isLoading ? (
                        ...
                        filtered.map((tx) => (
                            ...
                            >
                                {/* Type badge */}
                                <div
                                    className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${tx.type === "in"
                                        ? "bg-emerald-50"
                                        : "bg-red-50"
                                        }`}
                                >
                                    <Icon
                                        ...
                                        className={`text-base ${tx.type === "in"
                                            ? "text-emerald-500"
                                            : "text-red-400"
                                            }`}
                                    />
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm font-semibold text-gray-800">
                                        ...
                                        <span
                                            className={`font-bold ${tx.type === "in"
                                                ? "text-emerald-600"
                                                : "text-red-500"
                                                }`}
                                        >
            ...
            <Modal.Footer className="border-t border-gray-100 px-8 pb-6 pt-4">
                <Button
                    color="gray"
                    onClick={onClose}
                    className="w-full rounded-2xl h-11 border-none bg-gray-100 hover:bg-gray-200"
                >
                    ปิด
                </Button>
            </Modal.Footer>
